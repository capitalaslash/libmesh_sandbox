...
void inlet(DenseVector<Real> & output,
           const Point & /*p*/,
           const Real /*time*/)
{
  output(0) = 0.;
...
void assemble_stokes(EquationSystems& es, const std::string& system_name)
{
  ...
  TransientLinearImplicitSystem& system = es.get_system<TransientLinearImplicitSystem> (system_name);
  ...
  for ( ; el != end_el ; ++el)
  {
    ...
    for (uint qp = 0; qp < qrule.n_points(); qp++)
    {
      ...
      {
        Fu(i) += JxW[qp] * (vel_old(0)*phi[i][qp] +
                            dt*f(0)*phi[i][qp] +
                            dt*(vel_old*gradu_old)*phi[i][qp]);
        Fv(i) += JxW[qp] * (vel_old(1)*phi[i][qp] +
                            dt*f(1)*phi[i][qp] +
                            dt*(vel_old*gradv_old)*phi[i][qp]);
        Fw(i) += JxW[qp] * (vel_old(2)*phi[i][qp] +
                            dt*f(2)*phi[i][qp] +
...
int main(int argc, char* argv[])
{
  ...
  const uint nx = (argc == 4)? atoi(argv[1]) : 2;
  const uint ny = (argc == 4)? atoi(argv[2]) : 2;
  const uint nz = (argc == 4)? atoi(argv[3]) : 2;

  MeshTools::Generation::build_cube(mesh, nx, ny, nz, 0., 1., 0., 1., 0., 1., HEX27);
  ...
  TransientLinearImplicitSystem& system = es.add_system<TransientLinearImplicitSystem>("NS");
  ...
  system.get_dof_map().add_dirichlet_boundary(
      DirichletBoundary({0, 1, 3, 5}, {u_var, v_var, w_var}, zero));
  // x == xmin - imposed velocity
  system.get_dof_map().add_dirichlet_boundary(
      DirichletBoundary({4}, {u_var}, ParsedFunction<Number>("16*y*(1-y)*z*(1-z)")));
  system.get_dof_map().add_dirichlet_boundary(
      DirichletBoundary({4}, {v_var, w_var}, zero));
...
//  const Real nonlinear_tolerance       = 1.e-5;
  ...
  es.parameters.set<Real>("linear solver tolerance") = 1.e-12;
  es.parameters.set<Real> ("dt")   = dt;
  es.parameters.set<Real> ("nu") = .007;
