...
void inlet_v(DenseVector<Number> & output, const Point & /*p*/, const Real /*time*/)
{
  output(1) = 1.;
}
...
void assemble_stokes(EquationSystems & es, const std::string & system_name)
{
  ...
  std::unique_ptr<FEBase> fe_side(FEBase::build(dim, fe_type_vel));
  ...
  DenseSubMatrix<Number> Kuu(Ke), Kuv(Ke), Kup(Ke), Kvu(Ke), Kvv(Ke), Kvp(Ke), Kpu(Ke),
      Kpv(Ke), Kpp(Ke);
      ...
      if (elem->neighbor_ptr(side) == NULL)
      {
        ...
        for (uint qp = 0; qp < qside.n_points(); qp++)
        {
          ...
          if (yf < 1e-6)
          {
            ...
            {
              // auto const JxW = JxW_side[qp];
              // auto const v = v_side[i][qp];
              Fv(i) += JxW_side[qp] * value * v_side[i][qp];
...
int main(int argc, char * argv[])
{
  ...
  MeshTools::Generation::build_square(mesh, nx, ny, 0., 1., 0., 1., QUAD9);
  ...
  // );
  // bottom side - imposed u
  system.get_dof_map().add_dirichlet_boundary(DirichletBoundary({0}, {u_var}, &zero));
  // top side - imposed u
  system.get_dof_map().add_dirichlet_boundary(DirichletBoundary({2}, {u_var}, &zero));
  // left side - symmetry
  system.get_dof_map().add_dirichlet_boundary(DirichletBoundary({3}, {u_var}, &zero));
  ...
  file_name << "out_" << std::setw(3) << std::setfill('0') << std::right << 0 << ".e";
