...
void assemble_advection(EquationSystems& es, const std::string& system_name)
{
    ...
    for ( ; el != end_el ; ++el)
    {
        ...
        for(uint qp = 0; qp < n_qp; qp++)
        {
            ...
            {
                Real test = v[i][qp];
                Real const velnormi = 1. / (vel * vel);
                test += 0.5 * 0.1 * (vel * dv[i][qp]) * velnormi;

                Fe(i) += JxW[qp] * old * test;

                for(uint j = 0; j < n_dofs; j++)
                {
                    Ke(i,j) += JxW[qp] * ( v[j][qp] * test +
                                           dt * vel * dv[j][qp] * test +
                                           dt * mu * dv[j][qp] * dv[i][qp] );
                }
            }
        }
...
int main(int argc, char* argv[])
{
    ...
    uint const nx = (argc == 2)? atoi(argv[1]) : 10;
    ...
    TransientLinearImplicitSystem& system = es.add_system<TransientLinearImplicitSystem>("Adv");
    ...
    Real const dt = 0.1;
    system.time = 0.0;
    uint const nsteps = 40;

    es.parameters.set<Real> ("dt") = dt;
    es.parameters.set<Real> ("vel") = 1.0;
    es.parameters.set<Real> ("mu") = 0.01;

    system.project_solution(ic, libmesh_nullptr, es.parameters);

    ExodusII_IO(mesh).write_timestep("out.e-s.000", es, 1, system.time);

    for (uint tstep = 1; tstep <= nsteps; ++tstep)
    {
      system.time += dt;
